...
class SettingsViewController: UIViewController,UITableViewDelegate,UITableViewDataSource{
    ...
    func setChat(chat:Chat,memberProfs:NSArray){
        //sync to saved settings of chat
        self.chat = chat
        if chat.joinType == 1{
            joinSwitch.isOn = false
        }
        members = memberProfs as! NSMutableArray
        ...
        for var i in 0..<FirebaseHelper.personal.friends.count{
            ...
            for var j in 0..<members.count{
                if (members[j] as! Profile).userId == (FirebaseHelper.personal.friends[i] as! Profile).userId{
                    isIn = true
                }
            }
            if !isIn{
                addArray.add((FirebaseHelper.personal.friends[i] as! Profile))
            }
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 1{
            if !members.contains(addArray.object(at: indexPath.row)){
                let button = UIButton(frame: CGRect(x: 0, y: 0, width: cell.frame.size.width, height: cell.frame.size.height))
                button.backgroundColor = .clear
                button.tag = indexPath.row
                button.addTarget(self, action: #selector(friendTapped(sender:)), for: .touchUpInside)
    ...
    @objc func friendTapped(sender:UIButton) {
        self.members.add(addArray.object(at: sender.tag))
